...
use App\Http\Requests\Front\Employee\DocumantRequest;
...
class DashboardController extends FrontBaseController
{
    ...
    { 
        ...
        $employee = DB::table('employees')
            ...
            ->get();
        $emp_designation = $employee[0]->designation;

        $des = DB::table('designation')
            ->select('department_id')
            ->where('id', '=', $emp_designation)
            ->get();
        $designation = $des[0]->department_id;

        $not = DB::table('noticeboards')
            ->select('department_id')
            ->where('id', '=', $designation)
            ->get();
        $notice = $not[0]->department_id;

        $this->homeActive = 'active';
        $this->noticeboards = Noticeboard::where('noticeboards.status', '=', 'active')->where('department_id', '=', $designation)->orWhere('department_id', '=', $notice)
            ->orderBy('id', 'DESC')
            ->get();
    ...
    public function profileEmp()
    {
     ...
     $this->department = Department::pluck('name', 'department.id as id');
       ...
       $this->designation = Designation::find($this->employee->designation); 
        $doc = []; 
        $docFiles = ['resume','offerLetter','joiningLetter','contract','IDProof'];

     
        foreach ($docFiles as $key=>$documents) {
           
            if(isset($this->employee->documents[$key]) && $this->employee->documents[$key]->status != 2){   
            $doc[$documents] = $this->employee->documents[$key]->document_url;
            }else{  
                $doc[$documents] = 0; 
            }
            if(isset($this->employee->documents[$key])){
                $doc[$documents.'1'] =  $this->employee->documents[$key]->status;
    ...
    // }
        ...
        return View::make('front.empProfile', $this->data);

    }

    public function profilePostEmp(DocumantRequest $request, $id)
    {
        ...
        $emailInfo = [
            ...
            'to' => $employee->company->email,
            'active_company' => $employee->company
        ];
        $fieldValues = ['LINK' => $link, 'NAME' => $employee->full_name];
          
         EmailTemplate::prepareAndSendEmail('DOCUMENT_VERIFY', $emailInfo, $fieldValues);
