...
as a'), "a.employee_id", "=", "employees.id")
    ...
    public function store(StoreRequest $request)
    {
        ...
        $admins = Admin::where('company_id', $this->employee->company_id)->select('email')->get();
        $adminemail[] = $admins[0]['email'];

        $employee[] = $this->employee->email;
        $teamlead = Employee::where('designation', '=', $this->employee->designation)->where('teamlead', '=', '1')->select('email')->get();
        $result = $teamlead->toArray();
        $teamleademail = [];
        $emailCount = count($result);
        for ($i = 0; $i < $emailCount; $i++) {
            $teamleademail[$i] = $result[$i]['email'];
        }
        $result = array_merge($employee, $teamleademail, $adminemail);
        ...
        Mail::to(
            $result
        )->send(new LeaveRequest($this->data));
