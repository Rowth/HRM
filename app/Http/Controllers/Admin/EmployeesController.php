    ...
    public function ajax_employees()
    {
        ...
        return DataTables::of($result)
            ...
            ->editColumn('leave_type', function ($row) {
                $total_leave = Leavetype::get()->sum('num_of_leave') ;
                $leaveLeft = array_sum(Attendance::absentEmployee($row->id)) . '/' . $total_leave;
                   return $leaveLeft;
            })
//
    ...
    public function store(StoreRequest $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('profile_image')) {
                ...
                $employee->teamlead =$request->teamlead;
                $employee->save();
            }
        } catch (\Exception $e) {
            ...
            $documents = ['resume', 'offerLetter', 'joiningLetter', 'contract', 'IDProof','dmc', 'IDProof2', 'SchoolCertificates','TrainingCertificate'];
    ...
    public function edit(EditRequest $request, $id)
    {
        ...
        foreach ($this->employee->documents as $documents) {
            ...
            $doc[$documents->type.'1'] = $documents->status;
        }
    ...
    public function update(UpdateRequest $request, $id)
    {
        ...
        if ($request->updateType == 'company') {
            ...
            $companyDetails->teamlead =$request->teamlead;
        ...
        if ($request->updateType == 'documents') {
            // UPLOAD THE DOCUMENTS  -----------------
            $documents = ['resume', 'offerLetter', 'joiningLetter', 'contract', 'IDProof','dmc', 'IDProof2', 'SchoolCertificates'];
    ...
    public function updateDocuent($id,$type,$status)
    {

         Employee_document::where('employee_id', $id)->where('type',$type)->update(array('status' => $status));
        ...
        $fieldValues = ['LINK' => $link, 'NAME' => $employee->full_name, 'DOCNAME' => $type, 'STATUS' => $status==1?'Approved':'Not Approved please contact to your HR manager'];

         EmailTemplate::prepareAndSendEmail('DOCUMENT_STATUS', $emailInfo, $fieldValues);
                     //return Reply::success('Document Upload Succefully. Please Wait For Confirmation Form HR Manager');

        return Redirect::back()->withSuccess('Update Successfully. Email Has Been Sent');
    ...
    public function export()
    {
		...
		$columns = array("Employee ID", "Name", "Email", "Gender", "Department", "Designation", "Father's Name", "Mobile Number",
        ...
        foreach ($employees as $key => $employee) {
			...
			fputcsv($file, array($data[$key]['employeeID'], $data[$key]['full_name'], $data[$key]['email'], $data[$key]['gender'], $data[$key]['name'],$data[$key]['designation'],$data[$key]['father_name'],$data[$key]['mobile_number'],$data[$key]['date_of_birth'],$data[$key]['joining_date'],$data[$key]['local_address'],$data[$key]['permanent_address'],$data[$key]['status'],$data[$key]['exit_date'] ,
					$data[$key]['account_name'] ,$data[$key]['account_number'] ,$data[$key]['bank'] ,$data[$key]['tax_payer_id'] ,$data[$key]['branch'],$data[$key]['bin']));
