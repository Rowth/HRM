...
class PayrollsController extends AdminBaseController
{
    ...
    public function check()
    {
        ...
        if ($this->payrolls) {
            ...
            $this->days =  $this->countDays(request()->get('year'), request()->get('month'), array(0, 6));
            $this->attendance_count = Attendance::where('employee_id',request()->get('employee_id'))->whereMonth('date', request()->get('month'))
            ->whereYear('date', request()->get('year'))->count();
            $this->attendance_salary = ($this->basicSalary/$this->days)*$this->attendance_count;
            $output['success'] = 'fail';
