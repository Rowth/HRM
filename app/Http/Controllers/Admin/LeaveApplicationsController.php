...
class LeaveApplicationsController extends AdminBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if($inputs['application_status']=='rejected'){
            $inputs['rejection_reason'] = $inputs['rejection_reason'];
        }
        else{
            $inputs['rejection_reason'] = Null;
        }
        ...
        $teamlead = Employee::where('designation','=',$employee->designation )->where('teamlead','=', '1')->select('email')->get();
        ...
        if ($company->leave_notification == 1) {
            if ($request->application_status != 'pending') {
                $emailInfo = ['from_email' => $this->setting->email,
                    ...
                    'to' =>(array_merge($email,$teamleademail)),
                ...
                $fieldValues = [
                    ...
                    'REJECTED REASON' => $inputs['rejection_reason']
