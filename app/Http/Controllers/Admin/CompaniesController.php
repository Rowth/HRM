...
class CompaniesController extends AdminBaseController
{
    ...
    {
        $select = [
            'companies.id', 'logo', 'company_name', \DB::raw('MAX(`admins`.`number_of_logins`) as number_of_logins'), 'license_expired',
    ...
    {
        $data = $request->all();
        $loop_length  = count($data['testemaill']);
        ...
        // print_r($data['testpassword'][0]);die;
        for ($i = 0; $i < $loop_length; $i++) {

            $admin = Admin::create($data);
            $admin->email = $data['testemaill'][$i];
            $admin->password = Hash::make($data['testpassword'][$i]);
